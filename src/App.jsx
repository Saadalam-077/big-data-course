...
function App() {
  ...
  useEffect(() => {
    loadStudents();
    const saved = localStorage.getItem('bigDataUser');
    if (saved) {
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem('bigDataUser', JSON.stringify(user));
    } else {
      localStorage.removeItem('bigDataUser');
    }
  }, [user]);
