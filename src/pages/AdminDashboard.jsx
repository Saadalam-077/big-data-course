...
const AdminDashboard = ({ students, onNavigate, onLogout, onRefresh }) => {
  ...
  const labs = [
    { lab: 1, titleEn: "Introduction to Big Data" },
    { lab: 2, titleEn: "Big Data Characteristics (5 V's)" },
    { lab: 3, titleEn: "Hadoop Ecosystem" },
    { lab: 4, titleEn: "HDFS - Hadoop File System" },
    { lab: 5, titleEn: "MapReduce Fundamentals" },
    { lab: 6, titleEn: "Introduction to Apache Spark" },
    { lab: 7, titleEn: "Spark RDDs and DataFrames" },
    { lab: 8, titleEn: "NoSQL Databases" },
    { lab: 9, titleEn: "Data Visualization" },
    { lab: 10, titleEn: "Big Data Project" },
  ];
  ...
  const getGradeColor = (g) => {
    ...
    if (g === 'B+' || g === 'B') return 'text-purple-400';
  ...
  const getGradeBg = (g) => {
    ...
    if (g === 'B+' || g === 'B') return 'bg-purple-500/20 border-purple-500/50';
  ...
  const getScoreColor = (s) => {
    ...
    if (s >= 75) return 'text-purple-400';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/30 to-slate-900">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-purple-300/70 text-xs">Big Data | لوحة تحكم المدير</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800/50 rounded-2xl p-5 border border-slate-700">
            ...
            <p className="text-3xl font-bold text-pink-400">{Math.max(0, sections.length - 1)}</p>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden">
          ...
          {filteredStudents.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <button onClick={handleRefresh} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm">Refresh Data</button>
            </div>
          ) : (
                  ...
                  {filteredStudents.map(student => {
                    ...
                    return (
                      <tr key={student.id || student.studentNumber} className="hover:bg-slate-700/30">
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 bg-slate-700 rounded text-purple-400 text-sm">{student.sectionNumber}</span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-2">
                            <div className="w-24 h-2 bg-slate-700 rounded-full overflow-hidden">
                              <div className="h-full bg-gradient-to-r from-purple-500 to-pink-400" style={{ width: `${pct}%` }}></div>
                            </div>
      ...
      <footer className="py-6 text-center border-t border-slate-800 mt-8">
        <p className="text-slate-500 text-sm">Big Data | Taif University</p>
      </footer>
    </div>
  );
};
