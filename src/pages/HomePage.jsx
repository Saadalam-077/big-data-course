...
const HomePage = ({ user, onNavigate, onLogout }) => {
  const labs = [
    { lab: 1, title: "Introduction to Big Data", titleAr: "مقدمة في البيانات الضخمة", icon: "📊" },
    { lab: 2, title: "Big Data Characteristics (5 V's)", titleAr: "خصائص البيانات الضخمة", icon: "🔄" },
    { lab: 3, title: "Hadoop Ecosystem", titleAr: "نظام Hadoop البيئي", icon: "🐘" },
    { lab: 4, title: "HDFS - Hadoop File System", titleAr: "نظام ملفات Hadoop", icon: "📁" },
    { lab: 5, title: "MapReduce Fundamentals", titleAr: "أساسيات MapReduce", icon: "🗺️" },
    { lab: 6, title: "Introduction to Apache Spark", titleAr: "مقدمة في Apache Spark", icon: "⚡" },
    { lab: 7, title: "Spark RDDs and DataFrames", titleAr: "RDDs و DataFrames", icon: "📋" },
    { lab: 8, title: "NoSQL Databases", titleAr: "قواعد بيانات NoSQL", icon: "🗄️" },
    { lab: 9, title: "Data Visualization", titleAr: "تصور البيانات", icon: "📈" },
    { lab: 10, title: "Big Data Project", titleAr: "مشروع البيانات الضخمة", icon: "🚀" },
  ];
  ...
  const getGradeColor = (grade) => {
    ...
    if (grade === 'B+' || grade === 'B') return 'text-purple-400';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/30 to-slate-900">
      <header className="bg-slate-800/80 backdrop-blur border-b border-slate-700 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white font-bold">Big Data</h1>
              <p className="text-purple-300/70 text-xs font-arabic">البيانات الضخمة</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={() => onNavigate('profile')} className="flex items-center gap-2 px-3 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg">
              <div className="w-8 h-8 bg-purple-600 rounded-full flex items-center justify-center text-white font-bold text-sm">{user.studentName.charAt(0)}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Telegram Banner */}
        <a href="https://t.me/+yBUQ8BkQ0OtmZjE8" target="_blank" rel="noopener noreferrer" className="block mb-8 bg-gradient-to-r from-purple-600 to-pink-500 rounded-2xl p-4 hover:from-purple-500 hover:to-pink-400 transition-all">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
            ...
            <div className="mt-4 h-3 bg-slate-700 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-pink-400" style={{ width: `${progressPercent}%` }} />
            </div>
          </div>
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
          ...
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between"><span className="text-slate-500">Section:</span><span className="text-purple-400">{user.sectionNumber}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <p className="text-purple-300/70 font-arabic">معامل الدورة</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {labs.map((lab) => {
            ...
            return (
              <div key={lab.lab} onClick={() => onNavigate(`lab${lab.lab}`)} className={`p-5 rounded-xl border-2 cursor-pointer transition-all hover:scale-[1.02] ${isCompleted ? 'bg-emerald-900/20 border-emerald-500/50' : 'bg-slate-700/30 border-slate-600 hover:border-purple-500/50'}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl ${isCompleted ? 'bg-emerald-600' : 'bg-purple-600'}`}>{isCompleted ? '✓' : lab.icon}</div>
                  <div className="flex-1">
                    <span className="text-xs text-purple-400 font-semibold">Lab {lab.lab} | المعمل {lab.lab}</span>
                    <h4 className="text-white font-semibold text-sm">{lab.title}</h4>
                    <p className="text-purple-300/60 text-xs font-arabic">{lab.titleAr}</p>
                  </div>
                </div>
                {isCompleted ? (
                  <div className="mt-3 flex items-center justify-between bg-slate-800/50 rounded-lg p-2">
                    ...
                    <span className={`font-bold text-sm ${score >= 90 ? 'text-emerald-400' : score >= 60 ? 'text-purple-400' : 'text-red-400'}`}>{score}%</span>
                  </div>
                ) : (
        ...
        <div className="mt-12 bg-slate-800/50 rounded-2xl p-8 border border-slate-700 text-center">
          ...
          <p className="text-purple-300/70 font-arabic mb-6">التواصل والدعم</p>
          <div className="flex flex-col md:flex-row items-center justify-center gap-6">
            <a href="https://t.me/+yBUQ8BkQ0OtmZjE8" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-6 py-4 bg-purple-600 hover:bg-purple-500 rounded-xl transition-colors">
            ...
            <div>
              ...
              <p className="text-purple-300/70 font-arabic">سعد العمري</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <footer className="py-6 text-center border-t border-slate-800 mt-8">
        <p className="text-slate-500 text-sm">Big Data | Taif University | جامعة الطائف</p>
